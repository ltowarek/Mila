...
std::vector<mila::Point> mila::ParallelMeanShift::Run(const std::vector<Point> &points,
  ...
  const auto original_points_buffer = clpp::Buffer(ocl_app_->GetContext(), CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, output.size() * sizeof(output.at(0)), output.data());
  const auto current_points_buffer = clpp::Buffer(ocl_app_->GetContext(), CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, output.size() * sizeof(output.at(0)), output.data());
  const auto shifted_points_buffer = clpp::Buffer(ocl_app_->GetContext(), CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, output.size() * sizeof(output.at(0)), output.data());

  auto distances = std::vector<cl_float>(output.size(), 0.0f);
  const auto distances_buffer = clpp::Buffer(ocl_app_->GetContext(), CL_MEM_WRITE_ONLY, output.size() * sizeof(distances.at(0)));
...
void mila::ParallelMeanShift::SaveEvents(clpp::Event copy_buffer, clpp::Event read_buffer, clpp::Event enqueue_nd_range) {
