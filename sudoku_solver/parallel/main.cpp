...
int main(int argc, char **argv) {
    ...
    auto sudoku_solver_initial = mila::sudokusolver::parallel::SudokuSolverBasedOnFilesProfiler(config.platform_id, config.device_id);
    ...
    for (size_t i = 0; i < config.number_of_iterations; ++i) {
        auto sudoku_solver = mila::sudokusolver::parallel::SudokuSolverBasedOnFilesProfiler(config.platform_id, config.device_id);
